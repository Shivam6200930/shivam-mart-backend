...
class Usercontroller {
        ...
        if (name && email && password && password_confirm) {
          if (password === password_confirm) {
            ...
            const token = jwt.sign(
              {userID:saved_user._id},process.env.jwt_secret_key,{expiresIn: "7d"}
            );
            ...
            res.status(200).json({ message: "data saved",token:token,mail:"email send sucessful" });
          } else {
            res.send({
              status: "failed",
  ...
  static changeUserpassword = async (req, res) => {
    ...
    if (password && password_confirm) {
      if (password === password_confirm) {
        ...
        res.status(200).json({ message: "password are changed",mail:'you sucesfully change your password' });
      } else {
        res
          .status(400)
        ...
        if (User) {
          ...
          transporter.sendMail(mailOptions, (error, info) => {
            if (error) {
              ...
              console.log("Email sent: " + info.response);
              res.status(203).json({ status: "passed", message: "Email sent for your reset password" });
            }
          });
        } else {
  ...
  static userPasswordReset = async (req, res) => {
    try {
      ...
      if (decodedToken.userID === User._id) {
        if (password && password_confirm) {
          if (password === password_confirm) {
            ...
            res.status(200).json({ message: "Password changed", mail: 'Email sent successfully' });
          } else {
            res.status(403).json({
              status: "failed",
      ...
      console.error(err);
      res.status(404).json({ status: "failed", message: "Your link has expired" });
    }
  };
